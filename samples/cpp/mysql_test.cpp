 ...
 * Demonstrates CRUD operations and database connectivity using MySQL C++
 * Connector
 *
 * This program uses the MySQL C API wrapped in C++ classes for a more modern
 * approach.
 *
 * Compile: g++ -o mysql_test mysql_test.cpp -I"%MYSQL_INCLUDE%" -L"%MYSQL_LIB%"
 * -lmysqlclient Run: mysql_test.exe
 */
...
string getEnv(const char *key, const char *defaultValue) {
  const char *val = std::getenv(key);
  return (val && *val) ? string(val) : string(defaultValue);
}

/* Database configuration */
const string DB_HOST = getEnv("DB_HOST", "localhost");
const string DB_USER = getEnv("DB_USER", "appuser");
const string DB_PASSWORD = getEnv("DB_PASSWORD", "72Je!^NY06OPx$uW");
const string DB_NAME = getEnv("DB_NAME", "testdb");
const int DB_PORT =
    (!getEnv("DB_PORT", "").empty()) ? stoi(getEnv("DB_PORT", "3306")) : 3306;
...
public:
  ...
  string getServerInfo() { return mysql_get_server_info(conn); }
  ...
  unsigned long long getInsertId() { return mysql_insert_id(conn); }

  unsigned long long getAffectedRows() { return mysql_affected_rows(conn); }

  const char *getError() { return mysql_error(conn); }
};
...
public:
  MySQLResult(MYSQL *conn) { result = mysql_store_result(conn); }
  ...
  int numRows() const { return result ? (int)mysql_num_rows(result) : 0; }

  int numFields() const { return result ? mysql_num_fields(result) : 0; }

  MYSQL_ROW fetchRow() { return result ? mysql_fetch_row(result) : nullptr; }
...
void printSeparator(int width) { cout << string(width, '-') << endl; }

void printSuccess(const string &message) { cout << "[OK] " << message << endl; }
...
public:
  ...
  bool connect() {
    ...
    cout << "[OK] Connected to MySQL Server version: " << db->getServerInfo()
         << endl;
  ...
  void testSelect() {
    ...
    printSeparator(70);
    cout << left << setw(5) << "ID" << setw(20) << "Name" << setw(35) << "Email"
         << setw(5) << "Age" << endl;
    printSeparator(70);
    ...
    while ((row = result.fetchRow())) {
      cout << left << setw(5) << (row[0] ? row[0] : "N/A") << setw(20)
           << (row[1] ? row[1] : "N/A") << setw(35) << (row[2] ? row[2] : "N/A")
  ...
  void testInsert() {
    ...
    stringstream query;
    query << "INSERT INTO users (name, email, age) VALUES ('" << name << "', '"
          << email << "', " << age << ")";
  ...
  void testUpdate() {
    ...
    if (!db->query("UPDATE users SET age = age + 1 WHERE email = "
                   "'cpp.test@example.com'")) {
      return;
    }
  ...
  void testJoin() {
    ...
    string query = "SELECT o.id, u.name AS customer, p.name AS product, "
    ...
    printSeparator(90);
    cout << left << setw(5) << "ID" << setw(20) << "Customer" << setw(20)
         << "Product" << setw(8) << "Qty" << setw(12) << "Total" << setw(12)
         << "Status" << endl;
    printSeparator(90);
    ...
    while ((row = result.fetchRow())) {
      cout << left << setw(5) << (row[0] ? row[0] : "N/A") << setw(20)
           << (row[1] ? row[1] : "N/A") << setw(20) << (row[2] ? row[2] : "N/A")
           << setw(8) << (row[3] ? row[3] : "N/A") << "$" << setw(11)
           << (row[4] ? row[4] : "N/A") << setw(12) << (row[5] ? row[5] : "N/A")
           << endl;
    }
  ...
  void testAggregates() {
    ...
    // Total revenue
    if (db->query("SELECT SUM(total_price) AS revenue FROM orders WHERE status "
                  "= 'delivered'")) {
      MySQLResult result(db->getConnection());
      MYSQL_ROW row;
      if (result.isValid() && (row = result.fetchRow())) {
        cout << "Total revenue (delivered orders): $"
             << (row[0] ? row[0] : "0.00") << endl;
      }
    }
  ...
  void cleanup() {
    ...
    cout << "[OK] Cleaned up " << db->getAffectedRows() << " test user(s)"
         << endl;
  }
...
int main(int argc, char *argv[]) {
  ...
  cout << "+============================================================+"
       << endl;
  cout << "|           C++ MySQL Connectivity Test                      |"
       << endl;
  cout << "|           Testing CRUD Operations with OOP                 |"
       << endl;
  cout << "+============================================================+"
       << endl;
  ...
  try {
    ...
    cout << "+============================================================+"
         << endl;
    cout << "|           All C++ MySQL Tests Completed!                   |"
         << endl;
    cout << "+============================================================+"
         << endl;

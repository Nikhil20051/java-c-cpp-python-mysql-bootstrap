 ...
 *
 * Compile: gcc -o mysql_test mysql_test.c -I"%MYSQL_INCLUDE%" -L"%MYSQL_LIB%"
...
const char *DEFAULT_USER = "appuser";
const char *DEFAULT_PASS = "72Je!^NY06OPx$uW";
const char *DEFAULT_NAME = "testdb";
...
void print_success(const char *message) { printf("[OK] %s\n", message); }

/* Print error message */
void print_error(const char *message) { printf("[ERROR] %s\n", message); }
...
int connect_to_database(void) {
  ...
  /* Fetch credentials from environment or defaults */
  const char *db_host = get_env_or_default("DB_HOST", DEFAULT_HOST);
  const char *db_user = get_env_or_default("DB_USER", DEFAULT_USER);
  const char *db_pass = get_env_or_default("DB_PASSWORD", DEFAULT_PASS);
  const char *db_name = get_env_or_default("DB_NAME", DEFAULT_NAME);

  const char *db_port_str = getenv("DB_PORT");
  int db_port = db_port_str ? atoi(db_port_str) : DEFAULT_PORT;
...
void test_select(void) {
  ...
  while ((row = mysql_fetch_row(result))) {
    printf("%-5s %-20s %-35s %-5s\n", row[0] ? row[0] : "N/A",
           row[1] ? row[1] : "N/A", row[2] ? row[2] : "N/A",
           row[3] ? row[3] : "N/A");
    num_rows++;
  }
...
void test_update(void) {
  ...
  const char *query =
      "UPDATE users SET age = age + 1 WHERE email = 'c.test@example.com'";
...
void test_join(void) {
  ...
  const char *query = "SELECT o.id, u.name AS customer, p.name AS product, "
...
void test_aggregates(void) {
  ...
  if (mysql_query(
          conn,
          "SELECT AVG(age) AS avg_age FROM users WHERE age IS NOT NULL") == 0) {
    result = mysql_store_result(conn);
    if (result && (row = mysql_fetch_row(result))) {
  ...
  /* Total revenue */
  if (mysql_query(conn, "SELECT SUM(total_price) AS revenue FROM orders WHERE "
                        "status = 'delivered'") == 0) {
    result = mysql_store_result(conn);
    if (result && (row = mysql_fetch_row(result))) {
  ...
  /* Products inventory */
  if (mysql_query(conn, "SELECT SUM(quantity) AS total_stock FROM products") ==
      0) {
    result = mysql_store_result(conn);
    if (result && (row = mysql_fetch_row(result))) {
